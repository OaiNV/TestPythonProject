...
from CEC_DocifyCode_Common.repositories.basic_design_repository import basic_design_repository
...
class BasicDesignService:
    ...
    def __init__(self):
        if not BasicDesignService._initialized:
            self.repository = basic_design_repository
    ...
    async def create_basic_design_from_file(
        ...
        try:
            ...
            if duplicate_file:
                ...
                is_inserted = await self.repository.insert_document(basic_design_doc)
                ...
                files.append(
                    {
                        ...
                        "status": (FileStatus.GIT if commit_id else FileStatus.LOCAL),
                        "git": {
                            "commit_id": commit_id,
    ...
    async def get_basic_design_detail(
        ...
        logger.info(
            f"[get_basic_design_detail] Start - project_id={project_id}, file_id={file_id}, user_id={user_id}"
        )
        ...
        try:
            ...
            if not document:
                logger.warning(
                    f"[get_basic_design_detail] Basic design not found - file_id={file_id}"
                )
                raise_http_error(status.HTTP_404_NOT_FOUND)

            logger.info(
                f"[get_basic_design_detail] Document fetched - basic_design_id={file_id}"
            )
            response = self._build_basic_design_detail_response(document)
            logger.info(
                f"[get_basic_design_detail] Success - basic_design_id={response.get('id')}"
            )
            return response
    ...
    async def download_basic_designs(
        ...
        """Download basic design files: single file (no zip) or multiple files (zip).
        Automatically includes referenced images from markdown files."""
        logger.info(
            f"[download_basic_designs] Start - project_id={project_id}, file_ids={file_ids}, user_id={user_id}"
        )
        ...
        try:
            ...
            documents = await self._fetch_basic_design_documents(project_id, file_ids)

            normalized_documents = [
                self.repository.ensure_document_id(document) for document in documents
            ]

            initial_count = len(normalized_documents)
            logger.info(
                "[download_basic_designs] Initial documents - count=%s, ids=%s",
                initial_count,
                [doc.get("id") for doc in normalized_documents],
            )

            # Check if single file and is image
            is_single_image = False
            referenced_images = []
            if len(normalized_documents) == 1:
                doc = normalized_documents[0]
                content_type = doc.get("contents", {}).get("type")
                if content_type == ContentType.IMAGE:
                    is_single_image = True

            if not is_single_image:
                # Fetch referenced images from markdown documents
                markdown_documents = [
                    doc
                    for doc in normalized_documents
                    if doc.get("contents", {}).get("type") == ContentType.MARKDOWN
                ]
                ...
                if markdown_documents:
                    referenced_images = await fetch_referenced_images(
                        ...
                        collection_type="basic_design",
                    )

                logger.info(
                    "[download_basic_designs] Referenced images - count=%s, ids=%s",
                    len(referenced_images),
                    [img.get("id") or img.get("_id") for img in referenced_images],
                )

                # Add images to documents list (avoid duplicates)
                existing_ids = {doc.get("id") for doc in normalized_documents}
                for img_doc in referenced_images:
                    ...
                    if img_id and img_id not in existing_ids:
                        normalized_documents.append(
                            self.repository.ensure_document_id(img_doc)
                        )
                        existing_ids.add(img_id)

            document_count = len(normalized_documents)
            logger.info(
                "[download_basic_designs] Final documents - count=%s (initial=%s + images=%s)",
                document_count,
                initial_count,
            ...
            logger.info(
                "[download_basic_designs] Retrieved documents - project_id=%s, document_count=%s (including %s images)",
                project_id,
                document_count,
                len(referenced_images),
            )
            ...
            resolvers = download_service.create_resolver_config(
                ...
                content_extractor=extract_basic_design_content,
            )
    ...
    def _extract_contents_dict(self, document: Dict[str, Any]) -> Dict[str, Any]:
        """Extract contents dictionary with multiple fallbacks for legacy structures."""
    ...
    def _build_basic_design_detail_response(
        ...
        try:
            ...
            response = {
                "id": normalized_doc.get("id"),
                "project_id": normalized_doc.get("project_id"),
                "commit_id": normalized_doc.get("commit_id"),
                "sync_status": normalized_doc.get("sync_status"),
                "file_name": normalized_doc.get("file_name"),
                "bd_content": {
                    "type": normalized_doc.get("contents", {}).get("type", ""),
                    "content": normalized_doc.get("contents", {}).get("content", ""),
                },
                "created_at": normalized_doc.get("created_at"),
                "updated_at": normalized_doc.get("updated_at"),
            }
