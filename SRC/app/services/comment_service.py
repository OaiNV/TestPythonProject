...
from CEC_DocifyCode_Common.repositories.comment_repository import comment_repository
...
class CommentService:
    ...
    async def create_comment(self, project_id: str, user_id: str, content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            has_access = await self.project_service.check_user_project_access(user_id, project_id)
            if not has_access:
            ...
            if is_inserted:
                logger.info(f"[create_comment] Comment created successfully: {comment_id}")
                return {
    ...
    async def get_comments(self, project_id: str, page: int = 1, per_page: int = 20) -> Dict[str, Any]:
