    ...
    try:
        ...
        if temp_dir and os.path.exists(temp_dir):
            try:
                ...
                logger.info(
                    f"[rename_folder_and_push] Cleaned up temp directory: {temp_dir}"
                )
            except Exception as e:
                logger.warning(
                    f"[rename_folder_and_push] Failed to clean up temp directory: {e}"
                )
    ...
    try:
        ...
        if temp_dir and os.path.exists(temp_dir):
            try:
                shutil.rmtree(temp_dir)
                logger.info(
                    f"[rename_multiple_folders_and_push] Cleaned up temp directory: {temp_dir}"
                )
            except Exception as e:
                logger.warning(
                    f"[rename_multiple_folders_and_push] Failed to clean up temp directory: {e}"
                )
            ...
            try:
                ...
                if hasattr(repository, "update_document"):
                    ...
                    if directory_key == "src":
                        collection = await repository._resolve_source_code_collection()
...
async def _update_data_management_folder_name(
    ...
    try:
        ...
        data_management_doc = await data_management_repository.find_one(
            {
                "project_id": project_id,
                "type": data_type,
        ...
        if updated_count > 0:
            ...
            modified_count = await data_management_repository.update_one(
