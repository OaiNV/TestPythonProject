...
from CEC_DocifyCode_Common.repositories.unit_test_repository import unit_test_repository
...
async def clone_code_from_git(
    ...
    try:
        ...
        await _check_folders_have_files(repo_root, directory)
        ...
        if temp_dir and os.path.exists(temp_dir):
            try:
                ...
                logger.info(
                    f"[clone_code_from_git] Cleaned up temp directory: {temp_dir}"
                )
            except Exception as e:
                error_message = (
                    f"[clone_code_from_git] Failed to cleanup temp directory: {e}"
                )
...
def _prepare_clone_url(
    ...
    try:
        ...
        if repository_url.startswith("https://"):
            ...
            if len(url_parts) == 2:
                clone_url = f"https://{user_name}:{token_password}@{url_parts[0]}/{url_parts[1]}"
                logger.info("[_prepare_clone_url] Added credentials to HTTPS URL")
                return clone_url
        elif repository_url.startswith("http://"):
            # http://gitbucket.example.com/user/repo.git -> http://username:token@gitbucket.example.com/user/repo.git
            url_parts = repository_url.replace("http://", "").split("/", 1)
            if len(url_parts) == 2:
                clone_url = (
                    f"http://{user_name}:{token_password}@{url_parts[0]}/{url_parts[1]}"
                )
...
async def _process_and_save_files(
    ...
    try:
        ...
        await _create_and_save_folder_structure(
            project_id, file_info_by_folder, file_id_mapping, repo_name
        )
...
async def _check_folders_have_files(repo_root: str, directory: Any) -> None:
    """Check if folders already have files in cloned repo, raise error if any folder has files"""
    logger.info(f"[_check_folders_have_files] Start - repo_root={repo_root}")
...
async def _process_bd_folder(
    ...
    try:
        ...
        files = _find_all_files(folder_path)
        repository = BasicDesignRepository()
        ...
        for file_path in files:
            try:
                ...
                if file_ext == ".md":
                    ...
                    logger.info(
                        f"[_process_bd_folder] Processed markdown file - file_path={normalized_path}, file_ext={file_ext}"
                    )
                else:
                    # Image: encode to base64
                    encoded_content = base64.b64encode(file_content).decode("utf-8")
                    ...
                    logger.info(
                        f"[_process_bd_folder] Processed image file - file_path={normalized_path}, file_ext={file_ext}"
                    )
                ...
                error_message = (
                    f"[_process_bd_folder] Error processing file {file_path}: {e}"
                )
                logger.error(error_message)
                save_exception_log_sync(e, error_message, __name__)
        ...
        # Note: data_management is not created for basic_design
...
async def _process_pd_folder(
    ...
    """Process detail design folder and subfolders (cd, md, id, fs, ac)"""
    ...
    try:
        ...
        # Map subfolder paths to types (only 5 declared subfolders in pd)
        subfolder_types = {
            directory.cd: DETAIL_DESIGN_TYPE_CD,  # CD (Class design)
            directory.md: DETAIL_DESIGN_TYPE_MD,  # MD (Method design)
            directory.id: DETAIL_DESIGN_TYPE_ID,  # ID (Interface design)
            directory.fs: DETAIL_DESIGN_TYPE_FS,  # FS (Folder structure)
            directory.ac: DETAIL_DESIGN_TYPE_AC,  # AC (Activity diagram)
        }

        total_files_processed = 0

        # Process each declared subfolder (cd, md, id, fs, ac)
        for subfolder_path, design_type in subfolder_types.items():
            if not subfolder_path:
                logger.info(
                    f"[_process_pd_folder] Subfolder path is empty for type={design_type}, skipping"
                )
                continue

            # Combine subfolder path with pd_path if it's relative
            # If subfolder_path doesn't start with pd_path, combine them
            if not subfolder_path.startswith(pd_path):
                combined_path = f"{pd_path}/{subfolder_path}".replace("//", "/")
            else:
                combined_path = subfolder_path

            subfolder_full_path = _normalize_path(combined_path, repo_root)
            if not subfolder_full_path:
                logger.warning(
                    f"[_process_pd_folder] Subfolder path not found: {combined_path} (original: {subfolder_path}, type={design_type})"
                )
                continue

            # Check if subfolder is within pd folder
            if not subfolder_full_path.startswith(pd_folder_path):
                logger.warning(
                    f"[_process_pd_folder] Subfolder {combined_path} is not within pd folder {pd_path}, skipping"
                )
                continue

            # Get all files in subfolder including nested folders
            files = _find_all_files(subfolder_full_path)

            logger.info(
                f"[_process_pd_folder] Processing subfolder {subfolder_path} (type={design_type}) - Found {len(files)} files"
            )

            if not files:
                logger.info(
                    f"[_process_pd_folder] No files found in subfolder {subfolder_path} (type={design_type})"
                )
                continue
            ...
            for file_path in files:
                try:
                    ...
                    # Create DetailDesign with detail_design_document_id = null
                    ...
                    if file_ext in [".jpg", ".jpeg", ".png", ".gif", ".bmp", ".webp"]:
                        ...
                        logger.info(
                            f"[_process_pd_folder] Processed image file - file_path={normalized_path}, file_ext={file_ext}"
                        )
                    else:
                        ...
                        logger.info(
                            f"[_process_pd_folder] Processed text file - file_path={normalized_path}, file_ext={file_ext}"
                        )

                    # detail_design_document_id is null when creating new
                    # type is set based on which subfolder the file is in (cd, md, id, fs, ac)
                    detail_design = {
                        "id": detail_design_id,
                        "project_id": project_id,
                        "detail_design_document_id": "",  # null when creating new
                        "type": design_type,  # CD, MD, ID, FS, or AC (based on subfolder)
                        "file_name": file_name,
                        "file_path": normalized_path,  # Đường dẫn file
                    ...
                    await detail_design_repository.insert_detail_design(detail_design)

                    total_files_processed += 1
                    logger.info(
                        f"[_process_pd_folder] Saved file - file_path={normalized_path}, type={design_type}, file_type={file_type}, id={detail_design_id}"
                    )

                except Exception as e:
                    error_message = (
                        f"[_process_pd_folder] Error processing file {file_path}: {e}"
                    )
                    logger.error(error_message)
                    save_exception_log_sync(e, error_message, __name__)
        ...
        # Note: data_management is not created for detail_design
        logger.info(
            f"[_process_pd_folder] Success - Processed {total_files_processed} files total"
        )
...
async def link_unit_test_to_source_code(
    ...
    try:
        ...
        source_code_collection = (
            await source_code_service.repository._resolve_source_code_collection()
        )
        ...
        for folder_path_str, file_list in file_info_by_folder.items():
            ...
            for file_info in file_list:
                try:
                    ...
                    await unit_test_repository.insert_unit_test(document)
...
def _find_source_code_files(directory: str, exclude_docifycode: bool = False) -> list:
    ...
    try:
        for root, dirs, files in os.walk(directory):
            ...
            # Skip Docifycode directory when exclude_docifycode is True
            if exclude_docifycode:
                # Remove Docifycode directory at any level (case-insensitive)
                dirs_to_remove = [
                    d for d in dirs if d.lower() == DOCIFYCODE_DIRECTORY_NAME.lower()
                ]
                for d in dirs_to_remove:
                    dirs.remove(d)
                    logger.info(
                        f"[_find_source_code_files] Excluded Docifycode directory: {os.path.join(root, d)}"
                    )
