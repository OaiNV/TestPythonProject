...
async def check_local_changes(project_id: str) -> bool:
    ...
    try:
        ...
        bd_collection = await basic_design_repository._resolve_collection()
        bd_query = {
            "project_id": project_id,
            "$or": [
                {"sync_status": {"$in": local_change_statuses}},
        ...
        dd_collection = await detail_design_repository._resolve_collection()
        dd_query = {
            "project_id": project_id,
            "$or": [
                {"sync_status": {"$in": local_change_statuses}},
        ...
        src_collection = await source_code_repository._resolve_source_code_collection()
        src_query = {
            "project_id": project_id,
            "$or": [
                {"sync_status": {"$in": local_change_statuses}},
        ...
        ut_collection = await unit_test_repository._resolve_collection()
        ut_query = {
            "project_id": project_id,
            "$or": [
                {"sync_status": {"$in": local_change_statuses}},
        ...
        dm_collection = (
            await source_code_repository._resolve_data_management_collection()
        )
        ...
        try:
            ...
            if temp_dir and os.path.exists(temp_dir):
                try:
                    ...
                    logger.info(f"[change_git_branch] Cleaned up temp directory")
                except Exception as e:
                    error_message = (
                        f"[change_git_branch] Failed to clean up temp directory: {e}"
                    )
                    logger.warning(error_message)
