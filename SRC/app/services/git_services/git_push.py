    ...
    try:
        ...
        if temp_dir and os.path.exists(temp_dir):
            try:
                ...
                logger.info(
                    f"[check_conflicts_before_push] Cleaned up temp directory: {temp_dir}"
                )
            except Exception as e:
                error_message = f"[check_conflicts_before_push] Failed to cleanup temp directory: {e}"
                logger.warning(error_message)
                save_exception_log_sync(
                    e, error_message, __name__, level=LogLevel.WARNING
                )
...
async def push_files_to_git(
    ...
    try:
        ...
        logger.info(f"[push_files_to_git] Success - commit_id={commit_id}")
        ...
        if temp_dir and os.path.exists(temp_dir):
            try:
                shutil.rmtree(temp_dir)
                logger.info(
                    f"[push_files_to_git] Cleaned up temp directory: {temp_dir}"
                )
            except Exception as e:
                error_message = (
                    f"[push_files_to_git] Failed to cleanup temp directory: {e}"
                )
                logger.warning(error_message)
                save_exception_log_sync(
                    e, error_message, __name__, level=LogLevel.WARNING
...
def _prepare_clone_url(
    repository_url: str, user_name: str, token_password: Optional[str]
) -> str:
    """Prepare clone URL with credentials"""
    logger.info("[_prepare_clone_url] Start")

    if not user_name or not token_password:
        logger.warning("[_prepare_clone_url] Missing credentials")
    ...
    try:
        ...
        if temp_dir and os.path.exists(temp_dir):
            try:
                shutil.rmtree(temp_dir)
                logger.info(
                    "[fetch_remote_file_content] Cleaned up temp directory: %s",
                    temp_dir,
                )
            except Exception as e:
                error_message = (
                    "[fetch_remote_file_content] Failed to cleanup temp directory: %s"
                    % (e,)
                )
                logger.warning(error_message)
                save_exception_log_sync(
