...
from CEC_DocifyCode_Common.repositories.user_repository import user_repository
...
class UserService:
    """Service for user operations - Singleton pattern"""
    ...
    async def create_user_from_local_login(self, local_login_data: dict) -> bool:
        ...
        try:
            ...
            existing_user = await self.get_user_by_email(local_login_data["login_id"])
            if existing_user:
                logger.info(
                    f"[create_user_from_local_login] User already exists: {local_login_data.get('login_id')}"
                )
                return True

            # Generate UUID7 for user_id
            user_id = str(uuid7())
