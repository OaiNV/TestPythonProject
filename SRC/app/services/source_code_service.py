...
from CEC_DocifyCode_Common.repositories.source_code_repository import source_code_repository
from CEC_DocifyCode_Common.repositories.detail_design_repository import detail_design_repository
...
class SourceCodeService:
    """Service for source code management operations - Singleton pattern"""
    ...
    async def get_project_source_code_data(self, project_id: str) -> Dict[str, Any]:
        """Get folders + source code files from source_code collection"""
        logger.info(
        ...
        try:
            ...
            mapped_sources = [self._map_source_fields(doc) for doc in raw_sources]
            payload = self._compose_project_payload(
                data_type="source_code", folders=folders, items=mapped_sources
            )
            logger.info(
    ...
    async def get_source_detail(
        ...
        try:
            ...
            detail_documents = await self.detail_design_repository.get_detail_design_documents_by_source(
                project_id=project_id, source_code_id=file_id
            )

            detail_design_map = await self._build_detail_design_map(
                project_id=project_id, documents=detail_documents
            )
            ...
            unit_test_map = self._group_unit_tests(unit_test_documents)

            response_payload = self._compose_source_detail_payload(
                source_document=source_document,
                detail_design_map=detail_design_map,
    ...
    async def _download_documents(
        ...
        file_type, file_suffix = ("source code", "source")
    ...
    def _build_detail_design_entry(
        ...
        try:
            ...
            entry = {
                ...
                "activity_diagram_group": asset_bundle.get("activity_diagram_group"),
    ...
    def _group_unit_tests(
        ...
        try:
            ...
            for document in unit_tests:
                ...
                buckets[bucket_key][entry_key] = self._build_unit_test_entry(
        ...
        try:
            ...
            if collection_name == UNIT_TEST_DESIGN_COLLECTION:
                design_group = {
                    ...
                    or MOCK_DECISION_TABLE,
                    "test_pattern": document.get("test_pattern")
                    or document.get("sample_table")
                    or MOCK_TEST_PATTERN,
                }
            elif collection_name == UNIT_TEST_CODE_COLLECTION:
                code_group = {
                ...
                # Fallback for legacy docs without collection_name metadata
                design_group = {
                    "unit_test_design_id": design_id,
                    "unit_test_design_json": document.get("unit_test_design_json")
                    or document.get("sample_table"),
        ...
        try:
            ...
            classes_payload = [
                self._build_class_payload(
                    class_document, detail_design_map, unit_test_map
                )
                for class_document in source_document.get("classes", [])
            ]
            global_methods_payload = [
                self._build_global_method_payload(
                    global_method, detail_design_map, unit_test_map
                )
                for global_method in source_document.get("global_methods", [])
            ]
    ...
    def _build_class_payload(
        ...
        try:
            ...
            payload = {
                ...
                "class_name": class_document.get("class_name", ""),
                "class_content": class_document.get("class_content", ""),
                "detail_design": detail_design_map.get(
                    (class_id, None), MOCK_DETAIL_DESIGN
                ),
    ...
    def _build_global_method_payload(
        ...
        try:
            ...
            detail_entries = detail_design_map.get((None, method_id), [])
            activity_group = (
                detail_entries[0].get("activity_diagram_group")
                if detail_entries
                else None
            )
            base_payload["activity_diagram_group"] = activity_group
            logger.info(
