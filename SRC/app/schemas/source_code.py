...
class SourceClassDetail(BaseModel):
    ...
    detail_design: List[DetailDesignEntry] = Field(
        default_factory=list, description="Detail design data for the class"
    )
...
class GlobalMethodDetail(MethodDetail):
    activity_diagram_group: Optional[ActivityDiagramGroup] = Field(
        None, description="Activity diagram for the global method"
    )
