...
class DetailDesignListItem(BaseModel):
    """Detail design list item response schema (without content)"""
...
class DetailDesignListData(BaseModel):
    ...
    detail_design_list: List[DetailDesignListItem] = Field(
...
class DetailDesignDetailResponse(BaseModel):
    ...
    statusCode: int = Field(..., description="HTTP status code")
    message: str = Field(..., description="Response message")
    data: DetailDesignItem = Field(..., description="Detail design data")
...
class DetailDesignDownloadRequest(BaseModel):
    """Schema for downloading detail designs"""

    file_id: List[str] = Field(
        ..., min_items=1, description="List of detail design IDs to download"
    )
