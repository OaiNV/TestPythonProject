...
from app.utils.constants import SyncStatus, GitType
...
from app.utils.helpers import is_valid_file_extension, validate_file_size_and_type
    ...
    try:
        ...
        if directory_info:
            directory_paths = {
                ...
                "cd": directory_info.cd or "",
                "md": directory_info.md or "",
                "id": directory_info.id or "",
                "fs": directory_info.fs or "",
                "ac": directory_info.ac or "",
...
async def _update_files_for_directory(
    ...
    try:
        ...
        # Note: pd uses detail_design_repository, not basic_design_repository
        requirement_repo = RequirementDocumentRepository()
        repository_map = {
            "rd": (requirement_repo, "file_path"),
            "bd": (basic_design_repository, "file_path"),
            "pd": (
                detail_design_repository,
                "file_path",
            ),  # PD uses detail_design_repository
            "cd": (detail_design_repository, "file_path"),
            "md": (detail_design_repository, "file_path"),
            "id": (detail_design_repository, "file_path"),
            "fs": (detail_design_repository, "file_path"),
            "ac": (detail_design_repository, "file_path"),
            "src": (source_code_repository, "file_path"),
            "utd": (unit_test_repository, "file_path"),
            "utc": (unit_test_repository, "file_path"),
        }
        ...
        repository, file_path_field = repository_map[directory_key]
...
async def _find_file_by_path(
    ...
    Args:
        ...
        project_id: Project ID
        file_path: File path to search for (e.g., "BD/business.md", "PD/CD/class_design.md")
        directory_key: Directory key (rd, bd, pd, cd, md, id, fs, ac, src, utd, utc)
    ...
    try:
        ...
        # For rd, pd, bd, find by file_name only
        if directory_key in ["rd", "pd", "bd"]:
            # Parse file_path: "BD/business.md" -> file_name="business.md"
            if len(path_parts) >= 1:
                # Get file_name (last part)
                file_name = path_parts[-1]
                query_params["file_name"] = file_name
            else:
                logger.warning(
                    f"[_find_file_by_path] Invalid file_path for {directory_key}: {file_path}"
                )
                return None

        # For detail design subdirectories (cd, md, id, fs, ac), find by type and file_name
        # When path is like "PD/CD/class_design.md":
        # - PD matches directory "pd" -> use detail_design_repository
        # - CD (second part) maps to directory "cd" -> add type DETAIL_DESIGN_TYPE_CD
        # - file_name is "class_design.md"
        elif directory_key in ["cd", "md", "id", "fs", "ac"]:
            # Parse file_path: "PD/CD/class_design.md" -> type=DETAIL_DESIGN_TYPE_CD, file_name="class_design.md"
            if len(path_parts) >= 2:
                # Get file_name (last part)
                file_name = path_parts[-1]
                # Map directory_key to DETAIL_DESIGN_TYPE constants
                type_map = {
                ...
                if design_type:
                    query_params["type"] = design_type
                    query_params["file_name"] = file_name
                else:
                    logger.warning(
                        f"[_find_file_by_path] Unknown directory_key for detail design: {directory_key}"
                    )
                    return None
            else:
                logger.warning(
                    f"[_find_file_by_path] Invalid file_path for detail design: {file_path}"
                )
                return None

        # For unit test (utd, utc), find by collection_name and file_path
