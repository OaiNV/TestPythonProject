...
const getRequirementFile = async (projectId, fileId) => {
  ...
  try {
    const url = `${RD_CONFIG.API_BASE_URL}${RD_CONFIG.PROJECTS_ENDPOINT}/${projectId}/requirement-documents/${fileId}`;
...
const getRequirementDocumentsList = async (projectId) => {
  ...
  try {
    const url = `${RD_CONFIG.API_BASE_URL}${RD_CONFIG.PROJECTS_ENDPOINT}/${projectId}/requirement-documents`;
    const response = await window.APIClient.get(url);
    const responseData = await response.json();
 ...
 * @param {string} endpointPath - Endpoint path (e.g., "requirement-documents")
...
const renderPreview = async (content) => {
  try {
    ...
    if (!content || content.trim() === "") {
      ...
      rdState.previewContent = "";
      return;
    }

    // Resolve local images in markdown before parsing
    let resolvedContent = content;
    if (window.MarkdownImageResolver && rdState.projectId) {
      try {
        // Fallback: if file_path not available, create from collection prefix + file_name
        const filePath =
          rdState.fileData?.file_path ||
          (rdState.fileData?.file_name ? `RD/${rdState.fileData.file_name}` : "RD/");
        resolvedContent =
    ...
    rdState.previewContent = content;
...
/**
 * Update visibility of save/restore controls based on sync status
 */
const updateRestoreVisibility = (syncStatus) => {
  ...
  try {
    ...
    const restoreBtn = document.getElementById("rdRestoreBtn");
...
const loadFileData = async () => {
  if (!rdState.projectId || !rdState.fileId) return;

  window.BaseUtils.showLoading();

  try {
    const [fileData, projectData] = await Promise.all([
      getRequirementFile(rdState.projectId, rdState.fileId),
      getProject(rdState.projectId),
    ]);

    rdState.fileData = fileData;
    rdState.syncStatus = fileData.sync_status || "";
    updateRestoreVisibility(rdState.syncStatus);
    renderHeader(fileData, projectData);

    const rdContent = fileData.rd_content || {};
    const contentType = rdContent.type || "markdown";
    const content = rdContent.content || fileData.content || "";

    rdState.contentType = contentType;
    ...
    if (contentType === "image") {
      if (content && typeof content === "string" && content.trim() !== "") {
        ...
        const previewEl = document.getElementById("rdPreview");
        if (previewEl) {
          previewEl.innerHTML = `<p style="text-align: center; color: #999; padding: 20px;">${RD_MESSAGES.NO_PREVIEW_AVAILABLE || "No image available"
            }</p>`;
        }
      }
    } else {
...
export const loadGitFiles = async () => {
  if (!rdState.projectId) return;

  try {
    const files = await getRequirementDocumentsList(rdState.projectId);
    rdState.gitFiles = files || [];
...
const checkAndToggleSaveButton = () => {
  ...
  if (!rdState.isEditable) {
    saveBtn.style.display = "none";
    return;
  }

  const currentCode = codeEditor.value;
  const hasChanges = currentCode !== rdState.codeContent;
...
const handleExpandCode = () => {
  try {
    if (!rdState.expandListenersAttached) {
      ...
      rdState.expandListenersAttached = true;
    }
  } catch (error) {
    throw error;
  }
...
/**
 * Handle generate basic design button
 */
const handleGenerateBasicDesign = () => {
  try {
    const generateBtn = document.getElementById("generateBasicDesignBtn");
    if (!generateBtn) return;

    generateBtn.addEventListener("click", async () => {
      await onGenerateBasicDesign();
    });
...
const onRestoreFile = async () => {
  if (!rdState.projectId || !rdState.fileId) {
  ...
  try {
    const projectResponse = await getProject(rdState.projectId);
    ...
    const endpointPath = GitPushEndpointPath.REQUIREMENT;
    ...
    rdState.syncStatus = updatedStatus;
    if (rdState.fileData) {
      rdState.fileData.sync_status = updatedStatus;
    }
...
const init = async () => {
  try {
    ...
    rdState.projectId = projectId;
    rdState.fileId = fileId;
...
const extractIdsFromUrl = () => {
  try {
    ...
    const rdIndex = pathParts.indexOf("rd");

    if (projectIndex === -1 || rdIndex === -1) {
      return { projectId: null, fileId: null };
    }

    const projectId = pathParts[projectIndex + 1];
    const fileId = pathParts[rdIndex + 1];
...
const showImagePreview = (base64Content) => {
  ...
  try {
    ...
    if (previewEl) {
      ...
      rdState.previewContent = base64Content;
    }
...
const navigateBack = () => {
  if (rdState.projectId) {
    ...
    const tab = urlParams.get("tab") || "requirement";

    // Navigate back with tab parameter to restore the tab
    window.location.href = `/projects/${rdState.projectId}?tab=${tab}`;
  } else {
    window.location.href = "/projects";
  }
};
...
const onSaveCode = async () => {
  ...
  try {
    await saveRequirementFile(rdState.projectId, rdState.fileId, content);
    rdState.codeContent = content;
    ...
    console.error("[onSaveCode] Save error:", error);
  } finally {
    window.BaseUtils.hideLoading();
    await loadGitFiles();
  }
...
const onExpandCodeClick = (e) => {
  ...
  rdState.isCodeExpanded = !rdState.isCodeExpanded;
  rdState.isPreviewExpanded = false;

  container.classList.remove("code-expanded", "preview-expanded");

  if (rdState.isCodeExpanded) {
...
const onExpandPreviewClick = (e) => {
  ...
  rdState.isPreviewExpanded = !rdState.isPreviewExpanded;
  rdState.isCodeExpanded = false;

  container.classList.remove("code-expanded", "preview-expanded");

  if (rdState.isPreviewExpanded) {
...
/**
 * Handle generate basic design action
 */
const onGenerateBasicDesign = async () => {
  if (!rdState.projectId || !rdState.fileId) {
    window.showAlert(RD_MESSAGES.PROJECT_ID_OR_FILE_ID_MISSING, "error");
    return;
  }
  ...
  const confirmed = await window.confirm(
    RD_MESSAGES.BASIC_DESIGN_GENERATE_TITLE,
    RD_MESSAGES.BASIC_DESIGN_GENERATE_CONFIRM
  );
  ...
  window.BaseUtils.showLoading();
  try {
    window.showAlert(RD_MESSAGES.BASIC_DESIGN_GENERATE_IN_PROGRESS, "info");
  } catch (error) {
    window.showAlert(RD_MESSAGES.BASIC_DESIGN_GENERATE_FAILED, "error");
  } finally {
    window.BaseUtils.hideLoading();
  }
};
...
const updateGitBranchInfo = async (files) => {
  try {
    const projectData = await getProject(rdState.projectId);
    const projectInfo = projectData.data || projectData;
...
const handlePushButton = async () => {
  ...
  try {
    ...
    // Determine endpoint path (requirement-documents for rd_detail)
    const endpointPath = GitPushEndpointPath.REQUIREMENT;
    ...
    try {
      ...
      if (result && result.isConflict && result.conflictFiles) {
        const forcePullOverride = async () => {
          window.BaseUtils.showLoading();
          try {
            await pullGitData(rdState.projectId, userName, tokenPassword, true);
