...
// entry point
compilation_unit
    : BYTE_ORDER_MARK? extern_alias_directives? using_directives? global_attribute_section* (namespace_member_declarations | global_statement)* EOF
    ;
...
global_method_declaration // lamdas from C# 6
    : attributes? method_modifiers? return_type method_member_name type_parameter_list? 
      OPEN_PARENS formal_parameter_list? CLOSE_PARENS type_parameter_constraints_clauses? 
      (method_body | right_arrow throwable_expression ';')
    | return_type method_member_name OPEN_PARENS formal_parameter_list? CLOSE_PARENS 
      (method_body | right_arrow throwable_expression ';')
    ;
